...
# run_test <model> <name> <src> <runner:uni|emu2> <oracle-mode:exact|substr> <oracle> [cflags]
# The optional 7th arg adds compile flags (e.g. -fpc: soft-float, no 8087).
run_test() {
    local model="$1" name="$2" src="$3" runner="$4" omode="$5" oracle="$6" xcflags="${7:-}"
    local zm lib crt lopt d cmd out
    zm="$(model_zm "$model")"; lib="$(model_lib "$model")"; crt="$(model_crt "$model")"; lopt="$(model_link "$model")"
    d="$WORK/$model-$name"; mkdir -p "$d"
    if ! "$WCC" -bt=dos -0 -m"$model" $zm $xcflags -zastd=c99 $INC "test/$src" -fo="$d/t.obj" >"$d/cc.log" 2>&1; then
